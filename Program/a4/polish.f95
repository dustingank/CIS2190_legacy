...
program reverse_polish
    ...
    do
        ...
        ! memory allocation
        allocate(stack(algebraic_length))
        stack(head) = '%'
            ...
            select case (algebraic_expression(index:index))
                ...
                case('+','-','*','/','%','^','(', ')')
                    ...
                    elseif (algebraic_expression(index:index) == ')') then
                        do
                            if (check_priority(stack(head)) .eq. 0) then
                                exit
                            else
                        ...
                        do
                            if (check_priority(algebraic_expression(index:index)) > check_priority(stack(head))) then
                                call push(algebraic_expression, stack, index, head)
                                exit
                            else 
                                call pop(reverse_polish_expression, stack, head, polish_length)    
    ...
    subroutine complete_reverse_expression(reverse_polish_expression, stack, polish_length, head) 
        ...
        character, allocatable, intent(inout):: stack(:)
        ...
        do
            ...
            if(stack(head) == '(') then
                ...
                polish_length = polish_length + 1
                reverse_polish_expression(polish_length:polish_length) = stack(head)    
            end if
            head = head - 1 
        end do
